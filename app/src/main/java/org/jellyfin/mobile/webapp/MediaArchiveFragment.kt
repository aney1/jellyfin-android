...
import org.jellyfin.mobile.databinding.FragmentMediaArchiveBinding
import org.jellyfin.mobile.utils.BackPressInterceptor
import org.jellyfin.mobile.utils.applyDefault

/**
 * Hosts Media Archive in an in-app [android.webkit.WebView], layered over the web app like the
 * player. It is a top-level load (not an iframe) so Media Archive's `X-Frame-Options: DENY`
 * doesn't apply. A small close button returns to the web app.
 */
class MediaArchiveFragment : Fragment(), BackPressInterceptor {
    private var binding: FragmentMediaArchiveBinding? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return FragmentMediaArchiveBinding.inflate(inflater, container, false).also { binding = it }.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.mediaArchiveWebView.apply {
            ...
            webViewClient = object : WebViewClient() {
                override fun onPageFinished(view: WebView, url: String?) {
                    this@MediaArchiveFragment.binding?.swipeRefreshLayout?.isRefreshing = false
                }
            }
            installLocaleWorkaround(this)
            loadUrl(BuildConfig.MEDIA_ARCHIVE_URL)
        }

        // Pull-to-refresh reloads only the Media Archive page. Only arm it when scrolled to the
        // top so it doesn't hijack normal scrolling.
        binding.swipeRefreshLayout.apply {
            setColorSchemeResources(R.color.jellyfin_accent)
            setOnChildScrollUpCallback { _, _ -> binding.mediaArchiveWebView.scrollY > 0 }
            setOnRefreshListener {
                binding.mediaArchiveWebView.reload()
            }
        }
    ...
    /**
     * Media Archive's frontend crashes with "Invalid language tag: null" when it passes a
     ...
     * values to a valid tag before Media Archive's bundle runs, so its pages render.
     */
    private fun installLocaleWorkaround(webView: WebView) {
        if (!WebViewFeature.isFeatureSupported(WebViewFeature.DOCUMENT_START_SCRIPT)) return
        // This WebView only ever loads Media Archive, so allow the shim on all origins.
        WebViewCompat.addDocumentStartJavaScript(webView, LOCALE_WORKAROUND_SCRIPT, setOf("*"))
    }

    override fun onInterceptBackPressed(): Boolean {
        val webView = binding?.mediaArchiveWebView ?: return false
    ...
    private companion object {
        /**
         * Runs before Media Archive's scripts. Wraps the Intl constructors so a null/empty (or
        ...
        val LOCALE_WORKAROUND_SCRIPT = """
            (function () {
                ...
                // Media Archive reads a usable value at the source.
                try {
