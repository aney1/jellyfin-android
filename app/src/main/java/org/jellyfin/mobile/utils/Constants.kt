...
object Constants {
    ...
    const val PREF_EXOPLAYER_ALLOW_MINI_PLAYER = "pref_exoplayer_allow_mini_player"
    const val PREF_EXOPLAYER_SHOW_THIN_PROGRESS_BAR = "pref_exoplayer_show_thin_progress_bar"
    const val PREF_EXOPLAYER_ALLOW_FREEFORM_ZOOM = "pref_exoplayer_allow_freeform_zoom"
    const val PREF_EXOPLAYER_SHOW_FRAME_STEP_BUTTONS = "pref_exoplayer_show_frame_step_buttons"
    const val PREF_EXOPLAYER_SHOW_YOUTUBE_BUTTON = "pref_exoplayer_show_youtube_button"
    const val PREF_EXOPLAYER_ROTATE_PORTRAIT_VIDEOS = "pref_exoplayer_rotate_portrait_videos"
    const val PREF_WEB_PULL_TO_REFRESH = "pref_web_pull_to_refresh"
    const val PREF_WEB_TUBE_ARCHIVIST_TAB = "pref_web_tube_archivist_tab"
    ...
    const val SKIP_MEDIA_SEGMENT_BACK_BUTTON_TIMEOUT = 5000L
    const val SEEK_FEEDBACK_TIMEOUT_MS = 5000L
    ...
    const val MILLISECONDS_PER_SECOND = 1000f
